...
export async function POST(req: Request) {
  try {
    ...
    const dispositivo = await prisma.dispositivoAsistencia.findUnique({ where: { id: dispositivoId }, include: { empresa: { select: { logoUrl: true } } } });
    ...
    return NextResponse.json({ ok: true, logoUrl: dispositivo.empresa.logoUrl });
  } catch (e: any) {
